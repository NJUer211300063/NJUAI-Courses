...
public class CW5 {
	...
	public boolean isValidPizza(OWLClassExpression exp) {
		...
		b =  r.isEntailed(df.getOWLSubClassOfAxiom(exp, pizza));
		return b;
	}
	...
	public Set<QueryResult> filterNamedPizzas(Set<QueryResult> results) {
		...
		for (QueryResult result : results) {
			OWLIndividual individual = result.getIndividual(); // Assuming there is a method to retrieve the individual from QueryResult
			boolean isNamedPizza = r.isEntailed(df.getOWLClassAssertionAxiom(np, individual));
			if (isNamedPizza) {
	...
	public Set<OWLClassExpression> getAllSuperclassExpressions(OWLClass ce) {
		...
		Set<OWLClass> superClasses = r.getSuperClasses(ce, true).getFlattened();
		for (OWLClass superClass : superClasses) {
			Set<OWLClassExpression> classExpressions = o.getSubClassAxiomsForSuperClass(superClass)
					.stream()
					.map(OWLSubClassOfAxiom::getSubClass)
					.collect(Collectors.toSet());
        	restrictions.addAll(classExpressions);
    	}
